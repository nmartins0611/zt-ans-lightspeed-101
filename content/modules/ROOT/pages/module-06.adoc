...
+
    b. In the **Ansible Lightspeed** section, click the **Create playbook with Ansible Lightspeed** button

.   **Enter your prompt:**
+
    a. A text box will appear asking for a prompt
+
    b. Enter a description of what you want to automate (e.g., "for rhel hosts with sudo, Install cockpit using redhat.rhel_system_roles collection & Copy cockpit.conf.j2 to /etc/cockpit directory & Restart and enable service")
+
    c. Click **Submit**

.   **Review the outline:**
+
    a. Lightspeed will show an editable outline of the playbook structure
+
    b. You can edit the outline to adjust the playbook structure if needed
+
    c. Click **Generate Playbook** to create the full playbook

.   **Save the playbook:**
+
    a. The playbook will open as "Untitled"
+
    b. Press `CTRL+S` (Windows/Linux) or `CMD+S` (Mac) to save
+
    c. **Option A:** Save with the same name as the demo playbook (e.g., `demo_install_cockpit.yml`) to use the existing job template
+
    d. **Option B:** Save with a different name - you'll need to create a new job template in AAP after pushing to Git
